...
class PICOModel:
	...
	def predict(data):
		X, Mask = prepare_data(data)
				
		input_idx_seq = torch.tensor(X)
	...
	def __init__():
		...
		self.cui_to_concept, self.concept_to_cui = concept_cui_mapping(data) 
		# # create the vocabulary for the input 
		self.idx_to_cui, self.cui_to_idx = extract_target_vocab(data)
		...
		self.model = BERTCLassifierModelAspect(self.n_tgt_vocab, dropout=dropout)
		# model = nn.DataParallel(model, output_device=device)
		# model.to(device)
		self.model.load_state_dict(torch.load('../saved_models/aspect_full_model.pt'))
